...
export default function DashboardLayout({
  ...
  const linkClass = (path: string) =>
    `${pathname === path ? "bg-yellow-600 text-white" : "text-gray-300"} 
     p-2 rounded block`;

  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="w-64 bg-grey-900 text-white p-4 space-y-3">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <Link href="/dashboard/MaintenanceHistory" className={linkClass("/dashboard/MaintenanceHistory")}>Maintenance History</Link>
          ...
          <Link href="/dashboard/UserAccessManagement" className={linkClass("/dashboard/UserAccessManagement")}>Role Distribution System</Link>
          <Link href="/" className={linkClass("/")}>LogOut</Link>
        </nav>
      </aside>
      {/* Main Content */}
